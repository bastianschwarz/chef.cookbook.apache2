...
def changelog_prject
  ENV['CHANGELOG_PROJECT'].presence  'chef.cookbook.apache2'
end
...
namespace :documentation do
  ...
  task changelog: ['git:setup'] do
    ...
    Dir.chdir(branch_repo) do
      sh format("github_changelog_generator -u #{changelog_user} -p #{changelog_prject}  -t #{ENV['GH_TOKEN']} %<version>s", version: ("--future-release #{version_match[1]}" unless version_match.nil?))
  ...
  task changelog_release: ['git:setup'] do
    unless version_match.nil?
      sh "github_changelog_generator -u #{changelog_user} -p #{changelog_prject} -t #{ENV['GH_TOKEN']} --future-release #{version_match[1]}"
      sh 'git diff --exit-code CHANGELOG.md' do |ok|
